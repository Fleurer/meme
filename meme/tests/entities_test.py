...
class TestOrder(unittest.TestCase):
    ...
    def test_compute_balance_diff_for_create(self):
        ...
        balance_diff = bid.build_balance_diff_for_close(account)
        account.adjust(balance_diff)
        self.assertEqual(float(balance_diff.old_active), 9.6997)
        self.assertEqual(float(balance_diff.new_active), 10)
        self.assertEqual(float(balance_diff.new_frozen), 0)
        self.assertEqual(balance_diff.active_diff, bid.freeze_amount)
        self.assertEqual(balance_diff.frozen_diff, 0 - bid.freeze_amount)
